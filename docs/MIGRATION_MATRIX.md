...
|---|---|---|---|---|---|
| Isar method export | `src/isar/method.rs` exports core `Thm` results and admits failed exports with `goal_export_*` reasons. | Unknown hyps and method fallbacks still dominate runtime failures. | Method execution returns a structured `ProofOutcome`; kernel-trusted success carries a context-bound new-kernel theorem. | Keep the implemented classifier and token-owning acceptance path stable while checked HOL elaboration remains missing. | `verify_no_open_oracle_free_results_in_core_batch`; reports distinguish `TransitionalStrictClosed`, compat, open, admitted, and failed. |
...
| Theorem database acceptance | HOL/global/theory tables filter with `is_strict_closed_proved()` while reports label that legacy status `TransitionalStrictClosed`. | Transitional facts can be misread as new-kernel proof progress. | Final trusted tables accept only `KernelTrustedClosed` values carrying a `src/kernel::TrustedTheorem` bound to immutable theory/logic context; searchable and transitional databases preserve taint. | Keep exact-owner acceptance stable; connect production HOL storage only after checked elaboration, basis, and definition gates close. | Reports show `TransitionalStrictClosed: 1/125` and `KernelTrustedClosed: 0/125` until a real new-kernel HOL theorem exists. |
| Proofterm replay | `src/core/proofterm.rs` supports a small replay subset; `src/kernel` replays every current strict derivation with exact context stamps and exact accepted-token theorem references. | Replay success can still be confused with closed theorem acceptance; logic-basis axiom and conservative-definition replay are absent. | Replay is an independent check in the exact immutable owner ancestry, never a replacement for closed acceptance. | Keep recursive acceptance/dependency replay stable before adding object-logic derivations. | Existing replay/tampering, stale/sibling token, dependency-order/kind, and deterministic-ID attacks. |
| Parser / certifier | Strict `CProp` certification, immutable context stamping, and an unresolved data-only `SourceProposition` model exist, but legacy compatibility parsing remains widespread and loses source distinctions. | Dummy types, omitted source provenance, and meta/object connective loss still block trusted theorem construction; the source AST is not yet parser-integrated or elaborated. | Declaration-aware parsing resolves the source AST into checked judgments, constants, schemes, and authorized `HOL.Trueprop` positions in context-bound `CProp`s. | Keep the source AST outside theorem authority; next integrate parsing and checked elaboration. | Attack tests cover AST structure/non-conversion now and parser/name/type/Trueprop attacks when integration lands. |
| HOL loader | `HolTheoremDb` remains a searchable fact index with admitted, compat, and transitional strict facts. | Searchable or transitional facts can be mistaken for new-kernel trusted facts. | HOL loader keeps searchable/transitional facts separate from context-bound kernel-trusted facts. | Preserve the `TrueI`-only transitional guard while parser integration and checked source elaboration remain missing. | `KernelTrustedClosed` can increase only from a context-bound new-kernel theorem, never a legacy outcome label. |
...
2. Keep implemented immutable `SignatureId` / `TheoryId` propagation and exact
   mixed-context rejection stable.
3. Keep the implemented unique, dependency-aware, mutually exclusive
   `KernelTrustedClosed` acceptance gate stable.
4. Keep the implemented data-only source proposition AST unresolved and
   disconnected from theorem authority.
5. Next, integrate parsing and elaborate checked judgments, constants, and
   polymorphic schemes, including `HOL.Trueprop`, into `CProp : prop`.
