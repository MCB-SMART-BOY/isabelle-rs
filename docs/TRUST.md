...
Full de Bruijn-style trust requires all four. Current work is moving from T2
engineering into T4 replay expansion while keeping known T2 debts visible.
...
| Status | Meaning | Counts as proved lemma? |
|---|---|---|
| Closed proved theorem | `|- P`, no oracle footprint, no `tpairs` | Yes |
...
| Searchable fact | fact available to proof search; may be open/admitted/generated | No, unless also closed proved |
...
final Theory theorem table
  ...
  = accepts only is_closed_proved()
```

`SessionBuilder` and verification statistics should report closed proved theorem
counts, not raw indexed theorem entries.
...
4. `bicompose`, `bicompose_eresolve`, `subst_premise`.
...
- Use `is_closed_proved()`-derived counts for verified lemmas.
