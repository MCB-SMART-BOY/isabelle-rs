...
| Parser / certification | Legacy `CTerm::certify` and compat paths | Strict `RawTerm -> CTerm/CProp` certification plus an untrusted `SourceProposition` data model | Parser, Isar, HOL loader | The source AST preserves raw syntax but is not parser-integrated or declaration-resolved; many legacy call sites remain compatibility-certified | Integrate source parsing and checked proposition elaboration for the first `KernelTrustedClosed` HOL slice | P0 | Convert targeted paths; no best-effort trust |
