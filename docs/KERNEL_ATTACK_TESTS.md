...
| Count a compat, open, admitted, or axiom-accepted result as `StrictClosed` in verification reports | `ProofOutcome` classifies strict closed, compat closed oracle-free, open oracle-free, admitted, and failed outcomes separately; only `StrictClosed` increments proved counts | `src/isar/method.rs::{proof_outcome_classifies_strict_closed,proof_outcome_does_not_count_axiom_accepted_strict_result,proof_outcome_classifies_compat_closed_oracle_free,proof_outcome_classifies_open_oracle_free,proof_outcome_classifies_admitted_reason}` |
| Count a compat implication identity as the first strict vertical slice | strict `A ==> A` is covered separately from compat `A ==> A`; only the checked strict path counts as `StrictClosed` | `tests/kernel_rewrite_soundness.rs::strict_kernel_pure_imp_identity_closes`; `src/isar/method.rs::{strict_vertical_slice_pure_imp_identity,proof_outcome_counts_pure_imp_identity_as_strict_closed,proof_outcome_does_not_count_compat_identity_as_strict_closed}` |
...
| Treat Pure reflexivity or compatibility `refl` as proof of HOL object equality | `try_strict_hol_refl` has a separate `hol_object_refl` derivation, requires checked `HOL.eq` and checked input, rejects missing/dummy `HOL.eq` and compat terms, and proof replay rejects using Pure `reflexive` for `HOL.eq` | `src/hol/hol_loader.rs::{hol_eq_type_env_declares_object_equality_shape,hol_eq_checked_application_has_bool_type_and_hol_shape,true_def_rhs_is_checked_reflexive_hol_eq_over_bool_function,pure_eq_is_not_accepted_as_hol_eq_shape,hol_refl_bridge_accepts_checked_term,hol_refl_bridge_rejects_missing_hol_eq,hol_refl_bridge_rejects_dummy_typed_hol_eq,hol_refl_bridge_rejects_compat_term,hol_refl_bridge_rejects_pure_eq_substitution}`; `src/core/proofterm.rs::{pure_reflexive_replay_rejects_hol_object_equality,hol_object_refl_replay_succeeds}` |
...
- `bicompose` and `bicompose_eresolve` (⚠️ LEGACY CORE) should get explicit
  ...
  enabled. Strict `subst_premise` and strict conservative `bicompose` wrapper
  now have conservative coverage; legacy-core `subst_premise` and full
  legacy-core `bicompose` remain compatibility debt.
