...
use crate::{
    ...
    hol::{
        function::FunDef,
        hol_loader::{generate_datatype_lemmas, parse_datatypes},
...
impl TheoryProcessor {
    ...
    pub fn process_file(path: &Path) -> Result<Arc<Theory>, Vec<String>> {
        ...
        if processor.errors.is_empty() { Ok(processor.finalize()) } else { Err(processor.errors) }
    }
    ...
    fn process_function(&mut self, span: &CommandSpan) {
        ...
        if body.contains('\\')
            ...
            || body.contains("_tr'")
            || body.contains("_binder")
            || body.contains("fun_upd")
            || body.contains("map_fun")
            || body.starts_with('(')
        {
            return;
        }
    ...
    fn process_locale_class(&mut self, span: &CommandSpan) {
        use crate::hol::{
            axclass::classdef_to_axclass,
            class_system::{
                instance_to_lemmas, parse_instances, parse_subclasses, subclass_to_lemmas,
            },
            locale::{locale_to_lemmas, parse_locales},
        };
            ...
            "class" => {
                ...
                if let Some(eq_pos) = body.find('=') {
                    ...
                    let superclasses: Vec<String> = super_part
                        ...
                        .filter(|s| {
                            !s.is_empty() && !s.starts_with("fixes") && !s.starts_with("assumes")
                        })
                        .collect();

                    // Parse fixes and assumes from class body
                    let src =
                        span.tokens.iter().map(|t| t.source.as_str()).collect::<Vec<_>>().join(" ");
    ...
    fn process_datatype(&mut self, span: &CommandSpan) {
        ...
        let full_cmd = span.tokens.iter().map(|t| t.source.as_str()).collect::<Vec<_>>().join(" ");
        ...
        for def in &defs {
            ...
            if let Some(ref mut local) = self.local {
                ...
                for (ctor_name, args) in &def.constructors {
                    ...
                    local.declare_const(
                        ctor_name,
                        Typ::arrows(vec![Typ::base("nat"); arg_count], result_typ),
                    );
                }
            }
    ...
    fn process_goal(&mut self, span: &CommandSpan) {
        ...
        catch_unwind_silent(|| {
            if let Some(ref mut proof) = self.proof {
                ...
                if is_show {
                    proof.show(name, stmt);
                } else {
                    proof.have(name, stmt);
                }
            }
        });
    }
    ...
    fn process_qed(&mut self, span: &CommandSpan) {
        ...
        catch_unwind_silent(|| {
            if let Some(ref mut proof) = self.proof {
                if span.name == "done" {
                    proof.done();
                } else if span.name == "by" {
                    proof.by(span.body.trim());
                } else if span.name == "sorry" {
                    proof.sorry();
                }
                if proof.level() <= 2 {
                    self.lemma_count += 1;
    ...
    fn test_batch_scan_theories() {
        ...
        match builder.scan(theories_dir) {
            Ok(count) => {
                ...
                let mut failure_reasons: std::collections::HashMap<String, usize> =
                            ...
                            Ok((false, errs, thm_count, indexed, _lemmas)) => {
                                ...
                                for err in &errs {
                                    let reason = if err.contains("No equations") {
                                        "parse:fun"
                                    } else if err.contains("Failed to parse datatype") {
                                        "parse:datatype"
                                    } else if err.contains("Cannot start") {
                                        "mode:assertion"
                                    } else if err.contains("No introduction rules") {
                                        "parse:inductive"
                                    } else {
                                        "other"
                                    };
