...
impl DependencySet {
    ...
    pub(in crate::kernel) fn empty() -> Self {
        Self::default()
    }

    pub(in crate::kernel) fn insert_theorem(&mut self, theorem: TheoremId) {
        self.entries
...
impl TheorySnapshot {
    ...
    pub(in crate::kernel) fn has_ancestor(&self, ancestor: TheoryId) -> bool {
...
impl TrustedTheory {
    ...
    pub(in crate::kernel) fn contains_theorem(&self, expected: &TrustedTheorem) -> bool {
        let mut current = Some(self);
        while let Some(theory) = current {
...
mod tests {
    use super::*;
    use crate::kernel::{CTerm, Derivation, KernelRules, KernelThm, ProofContext, RawTerm, Term};
