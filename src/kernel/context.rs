...
impl ProofContext {
    ...
    pub(in crate::kernel) fn validate_cterm(&self, term: &CTerm) -> Result<(), KernelError> {
    ...
    pub(in crate::kernel) fn validate_cprop(&self, prop: &CProp) -> Result<(), KernelError> {
    ...
    pub(in crate::kernel) fn recertify_prop(&self, prop: &CProp) -> Result<CProp, KernelError> {
        let mut bounds = Vec::new();
        let ty = self.validate_checked(prop.term(), &mut bounds)?;
        if !ty.is_prop() {
            return Err(KernelError::NotProposition(ty));
