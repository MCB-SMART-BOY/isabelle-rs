...
pub(in crate::kernel) const SIGNATURE_DOMAIN: &[u8] = b"isabelle-rs/signature/v1";
pub(in crate::kernel) const THEORY_DOMAIN: &[u8] = b"isabelle-rs/theory/v1";
...
impl SignatureId {
    ...
    pub(in crate::kernel) fn from_digest(digest: [u8; 32]) -> Self {
        Self(digest)
    }
...
pub(in crate::kernel) struct CanonicalEncoder {
    hasher: Sha256,
}

impl CanonicalEncoder {
    pub(in crate::kernel) fn new(domain: &[u8]) -> Self {
        let mut encoder = Self { hasher: Sha256::new() };
        encoder.write_bytes(domain);
        encoder
    }

    pub(in crate::kernel) fn write_u8(&mut self, value: u8) {
        self.hasher.update([value]);
    }

    pub(in crate::kernel) fn write_u64(&mut self, value: u64) {
        self.hasher.update(value.to_be_bytes());
    }

    pub(in crate::kernel) fn write_fixed_bytes(&mut self, value: &[u8]) {
        self.hasher.update(value);
    }

    pub(in crate::kernel) fn write_bytes(&mut self, value: &[u8]) {
        self.write_u64(value.len() as u64);
        self.write_fixed_bytes(value);
    }

    pub(in crate::kernel) fn write_name(&mut self, name: &Name) {
        self.write_bytes(name.as_str().as_bytes());
    }

    pub(in crate::kernel) fn finish(self) -> [u8; 32] {
