...
pub struct Pure;
impl Pure {
    pub fn prop_type() -> Typ { Typ::base("prop") }
    pub fn imp_const() -> Term { Term::const_("Pure.imp", Typ::arrow(Typ::base("prop"), Typ::arrow(Typ::base("prop"), Typ::base("prop")))) }
    pub fn mk_implies(a: Term, b: Term) -> Term { Term::app(Term::app(Self::imp_const(), a), b) }
        ...
        match term {
            Term::App { func, arg } => match func.as_ref() {
                Term::App { func: inner, arg: a } => match inner.as_ref() {
                    Term::Const { name, .. } if name.as_ref() == "Pure.imp" => Some((a.as_ref(), arg.as_ref())),
                    _ => None,
                },
                _ => None,
            },
    ...
    pub fn mk_equals(typ: Typ, t: Term, u: Term) -> Term {
        Term::app(Term::app(Term::const_("Pure.eq", Typ::arrow(typ.clone(), Typ::arrow(typ, Typ::base("prop")))), t), u)
    }
    ...
    pub fn strip_imp_prems(term: &Term) -> (Vec<&Term>, &Term) {
        let mut prems = Vec::new(); let mut body = term;
        while let Some((a, b)) = Self::dest_implies(body) { prems.push(a); body = b; }
        (prems, body)
    }
