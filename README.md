...
| Strict kernel nucleus | `src/kernel` is the new TCB: no dummy type, no compat certification, separate `ProofObligation`, `TrustedTheory`, and `SearchFactDb`; includes primitive rules, `resolve1_match`, conservative `subst_premise`, and conservative `bicompose` wrapper. |
...
1. Stabilize strict `src/kernel` nucleus (15 primitives + `resolve1_match` + conservative `subst_premise` + conservative `bicompose` wrapper) with firewall checks and compatibility matrix, then design full `bicompose` / `bicompose_eresolve`.
