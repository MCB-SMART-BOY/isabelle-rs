...
impl CTerm {
    ...
    pub(crate) fn new(term: Term, context: ContextStamp) -> Self {
        CTerm { term, context }
    }
    ...
    ///
    /// This constructor is `pub(crate)` — only `src/kernel/` modules
    /// can call it. External code and upper-layer modules MUST use
    /// `ProofContext::certify_term` instead.
    pub(crate) fn from_certified_subterm(term: Term, context: ContextStamp) -> Self {
        CTerm { term, context }
    }
...
impl CProp {
    pub(crate) fn new(term: Term, context: ContextStamp) -> Result<Self, KernelError> {
        if !term.ty().is_prop() {
    ...
    pub(crate) fn from_checked_term(term: Term, context: ContextStamp) -> Self {
